...
import { Field, Input, Select, TextArea, Btn } from '@/components/ui/FormElements';
import { productsApi, categoriesApi } from '@/lib/api';
import { Product, Category } from '@/types';
import { useAuthStore } from '@/store/auth.store';
import { Icon } from "@uigovpe/components";

const emptyForm = { name: '', description: '', price: '', stock: '0', categoryIds: [] as string[] };
...
export default function ProductsPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await productsApi.list({ page, limit: 10, search, categoryId: catFilter });
  ...
  useEffect(() => {
    ...
    productsApi.favorites().then((r) => setFavorites(new Set(r.data.map((p: Product) => p.id))));
  }, [fetchData]);
  ...
  const handleDelete = async () => {
    setSaving(true);
    try { await productsApi.remove(selected!.id); setModal(null); fetchData(); }
    finally { setSaving(false); }
  };
  ...
  const handleImageUpload = async () => {
    ...
    setSaving(true);
    try { await productsApi.uploadImage(selected.id, imageFile); setModal(null); fetchData(); }
    finally { setSaving(false); }
  };
  ...
  const columns = [
    { key: 'name', label: 'Nome' },
    { key: 'price', label: 'Preço', render: (p: Product) => `R$ ${Number(p.price).toFixed(2)}` },
    { key: 'stock', label: 'Estoque' },
    {
      key: 'categories', label: 'Categorias',
      render: (p: Product) => p.categories?.map((c) => c.name).join(', ') || '-',
    },
    { key: 'owner', label: 'Dono', render: (p: Product) => p.owner?.name || '-' },
  ];
          ...
          <DataTable
            ...
            actions={(p) => (
              ...
              >
                ...
                {/* Botão de Imagem (Substituindo 📷) */}
                {/* Botão de Imagem (Câmera) */}
                <Btn
                  variant="secondary"
                  onClick={() => {
                    setSelected(p);
